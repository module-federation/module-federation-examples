...
const appsData = returnCommonAngularAppsData(
  Constants.commonConstantsData.commonAngularAppsData.messages.shellWelcome.replace('11', '15'),
);
...
appsData.forEach(
  (property: {
    ...
    const appName =
      property.path === Constants.commonConstantsData.home.toLowerCase()
        ? appsData[0].appNameText
        : appsData[1].appNameText;
    const headerText =
      property.path === Constants.commonConstantsData.home.toLowerCase()
        ? appsData[0].headerText
        : appsData[1].headerText;
    const paragraphText =
      property.path === Constants.commonConstantsData.home.toLowerCase()
        ? appsData[0].paragraphText
        : appsData[1].paragraphText;
      ...
      context(`Check ${appName} App Page`, () => {
        ...
        it('Check App is build and running', () => {
          ...
          basePage.checkElementHaveProperty({
            ...
            value:
              Constants.commonConstantsData.commonAngularAppsData.mdmfNavigationItemLogo
                .targetBlank,
          });
    ...
    describe('Angular 15 microfrontends lazy components', () => {
      context('Check Apps functionality', () => {
        ...
        it('Check fields validation', () => {
          ...
          basePage.checkElementWithTextPresence({
            selector: commonSelectors.commonAngularAppsSelectors.alertMessage,
            text: Constants.commonConstantsData.commonAngularAppsData.messages.requiresMessages
              .name,
          });
          basePage.checkElementWithTextPresence({
            selector: commonSelectors.commonAngularAppsSelectors.alertMessage,
