  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.envInfo}>
        ...
        <ul>
          <li><strong>API URL:</strong> {ENV?.API_URL || 'Not configured'}</li>
          <li><strong>Remote URL:</strong> {ENV?.REMOTE_URL || 'http://localhost:3001/remoteEntry.js (default)'}</li>
          <li><strong>Environment:</strong> {process.env.NODE_ENV || 'development'}</li>
        </ul>
      </div>
      
      <p>
        This example demonstrates how Module Federation can load remote components dynamically 
        with runtime environment variables. The remote URL and other configuration can be 
      ...
      <div style={styles.remoteContainer}>
        ...
        <React.Suspense fallback={
          <div style={styles.statusText}>Initializing remote component...</div>
        }>
          {FederatedComponent ? (
            <FederatedComponent />
          ) : (
