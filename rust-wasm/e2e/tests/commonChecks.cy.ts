  ...
  context('It Checks buttons functionality', () => {
    ...
    appsData.forEach((property: { buttonName: string; consoleMessage: string; index: number }) => {
      ...
      it(`Checks ${property.buttonName} button still functioning even when game already started and stopped`, () => {
        ...
        basePage.checkInfoInConsole(
          Constants.commonPhrases.rustWasmApp.consoleMessages.stopLoopMessage,
          StubTypes.beCalled,
          false,
          false,
        );
        basePage.checkInfoInConsole(
          property.consoleMessage,
          property.buttonName === Constants.elementsText.rustWasmApp.buttonsNames[0]
            ? StubTypes.beCalled
            : StubTypes.notToBeCalled,
          false,
          false,
        );
        basePage.clickElementWithText({
      ...
      it(`Checks game board triggered by ${
        Constants.elementsText.rustWasmApp.buttonsNames[property.index]
      } 
            can be updated by ${
              Constants.elementsText.rustWasmApp.buttonsNames[extraPropertyIndex]
            } `, () => {
        const consoleMessage: string =
          property.index === 1 ? appsData[2].consoleMessage : appsData[1].consoleMessage;

        basePage.skipTestByCondition(property.buttonName === appsData[0].buttonName);
        basePage.openLocalhost({
          number: 8080,
        });
        methodsPage.checkGameBoardAppearsByClick(
          Constants.elementsText.rustWasmApp.buttonsNames[property.index],
        );
      ...
      it(`Checks game triggered by ${property.buttonName} button can be started and stopped by Play button`, () => {
        ...
        methodsPage.checkGameBoardAppearsByClick(property.buttonName);
        basePage.checkInfoInConsole(
          Constants.commonPhrases.rustWasmApp.consoleMessages.startLoopMessage,
          StubTypes.notToBeCalled,
          false,
        );
        basePage.checkInfoInConsole(
