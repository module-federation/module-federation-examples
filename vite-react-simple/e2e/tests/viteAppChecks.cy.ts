...
describe('Vite React Simple', () => {
  context('It checks elements colors/functionality', () => {
    ...
    it('Checks counter in counter-texted button is not updated by click on webpack button', () => {
      ...
      basePage.checkElementWithTextPresence({
        selector: baseSelectors.tags.coreElements.button,
        text: Constants.elementsText.viteReactSimpleApp.buttons.counter.replace(
          Constants.commonConstantsData.commonIndexes.zero.toString(),
          Constants.commonConstantsData.commonIndexes.one.toString(),
        ),
        isVisible: false,
      });
      basePage.checkElementWithTextPresence({
