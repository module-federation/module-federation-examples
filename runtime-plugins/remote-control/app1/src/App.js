...
const App = () => (
  <div>
    ...
    {__FEDERATION__.__INSTANCES__.map((inst) => (
      <span style={{padding: 10, color: '#fff',background: getColorFromString(inst.name.split().reverse().join(''))}} key={inst.name}>{inst.name}</span>
    ))}
    <p>
      Click The second button. This will cause the <i>pick-remote.ts</i> to load remoteEntry urls from a mock api call.
    </p>
    ...
    <button onClick={()=> {localStorage.clear('button'); window.location.reload()}}>Reset </button>
  </div>
);
