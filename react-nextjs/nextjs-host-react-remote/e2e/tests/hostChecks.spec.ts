...
const CHANGED_CONTENT_PATH = path.resolve(__dirname, '../fixtures/changedContent.js');
...
const remoteComponentMessage =
  Constants.commonPhrases.nextJsHostReactRemoteApp.messages.remotes.component;
...
test.describe('Host page checks', () => {
  ...
  test('reflects root file content changes', async ({ page }) => {
    ...
    const initialContent = await readFile(ROOT_FILE_PATH, 'utf-8');
    const changedContent = await readFile(CHANGED_CONTENT_PATH, 'utf-8');

    try {
      await writeFile(ROOT_FILE_PATH, changedContent, 'utf-8');
      ...
      await writeFile(ROOT_FILE_PATH, initialContent, 'utf-8');
      await page.waitForTimeout(1000);
      await page.reload();
      await expect(introParagraph).toBeVisible();
    } finally {
