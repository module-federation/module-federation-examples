...
describe('It checks elements visibility', () => {
  ...
  CommonTestData.commonNextJsAppsData.forEach(
    (property: { messageType: string; selector: string; message: string; linkText?: string }) => {
      hosts.forEach((host: number) => {
        it(`Checks ${property.messageType} message visibility`, () => {
          basePage.skipTestByCondition(
            host === 8081 && property.message === CommonTestData.commonNextJsAppsData[3].message,
          );
