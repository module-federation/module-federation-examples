...
describe('Vite Vue Microfrontends', () => {
  context('It checks names and symbols inside cards', () => {
    CommonTestData.commonMicroFrontendsAppsData.forEach(
      (property: { cardName: string; symbolName: string; symbol: string; status: string }) => {
        it(`Checks ${property.symbolName} symbol visibility for ${property.cardName} card`, () => {
          ...
          basePage.checkElementVisibility({
            ...
            parentElement:
              property.symbolName === CommonTestData.commonMicroFrontendsAppsData[0].symbolName,
          });
        });
