...
describe('Typescript Monorepo', () => {
  context('It checks typescript-monorepo apps', () => {
    CommonTestData.commonTypeScriptAppsData.forEach(
      (property: { host: number; header: string; appName: string }) => {
        ...
        it(`Checks that apps names is not equal`, () => {
          ...
          basePage.compareInfoBetweenHosts(
            updatedSelectors.common.appName,
            property.host === 3002
              ? CommonTestData.commonTypeScriptAppsData[0].host
              : CommonTestData.commonTypeScriptAppsData[1].host,
            false,
          );
        });
      },
