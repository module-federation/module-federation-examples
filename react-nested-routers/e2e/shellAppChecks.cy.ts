...
describe('React Nested Routers', () => {
  context('It checks shell app', () => {
    ...
    it('Checks base page message & clicks each link by text & checks changes in page message and url & check reload does not revert changes', () => {
      ...
      Constants.elementsText.reactNestedRoutersApp.shellAppTextedLinks.forEach(
        (link: string, index: number) => {
          basePage.checkUrlText(
            index === 4
              ? Constants.elementsText.reactNestedRoutersApp.shellAppLinks[2]
              : Constants.elementsText.reactNestedRoutersApp.shellAppLinks[index],
            false,
          );
          if (index !== 0) {
            basePage.checkElementWithTextPresence({
