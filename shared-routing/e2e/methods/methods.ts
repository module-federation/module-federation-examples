...
export class SharedRoutingMethods extends BaseMethods {
  public checkInputWithLabelVisibilityInsideBlock(formField: string, text: string): void {
    cy.get(baseSelectors.tags.inputs.input)
      .parentsUntil(formField)
      .find(baseSelectors.tags.coreElements.label)
  ...
  public returnReplacedFormFieldSelector({
    ...
    if (index >= 0) {
      ...
      const replaceSelectorPart =
        index === 7
          ? Constants.selectorParts.sharedRoutingAppSelectorsParts.userInfo.toUpperCase()
          : Constants.elementsText.sharedRoutingApp.editProfileBlockLabels[index]
              .replace(/\s/g, '_')
              .replace(/([()])/g, '');
  ...
  public visitOnPageByName(
    checkedPageHeader: string,
    remotePageHeader: string,
    host: number,
  ): void {
    this.openLocalhost({
      number: host,
    ...
    this.clickElementBySelector({
      selector: baseSelectors.css.href.replace('{link}', `/${checkedPageHeader.toLowerCase()}`),
    });
    this.checkElementWithTextPresence({
      selector: baseSelectors.tags.headers.header,
      text: remotePageHeader,
  ...
  public transferringThroughPages(
    landingPageHeader: string,
    firstRemotePageHeader: string,
    secondRemotePageHeader: string,
  ): void {
    this.checkElementWithTextPresence({
      selector: baseSelectors.tags.headers.header,
    ...
    this.clickElementBySelector({
      selector: baseSelectors.css.href.replace('{link}', `/${firstRemotePageHeader.toLowerCase()}`),
    });
    this.checkElementWithTextPresence({
      selector: baseSelectors.tags.headers.header,
  ...
  public checkElementWithTextPresenceInTextArray({
    ...
    textArray.forEach(text => {
      if (childElement) {
        this.checkElementVisibility({
          parentSelector,
          selector: selector.replace('{cellType}', text.replace(/\s/g, '_').toUpperCase()),
        });
      } else {
        this.checkElementWithTextPresence({
          parentSelector,
  ...
  private _fillFieldAndCheckValue(
