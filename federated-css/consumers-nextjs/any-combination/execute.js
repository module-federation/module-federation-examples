...
const mfPluginConfig =
  configs => `const {getRemoteEntry, remotes} = require("../../expose-remotes/remotes.config");
...
const mapped = scopes.map(scope => scope.replace('@federated-css/', ''));
const pages = mapped.map(s => exposeScopesMap[s].page);
const mfConfigs = mapped.map(s => exposeScopesMap[s].mfConfig);

fs.writeFile(
  path.join(__dirname, '/components/pages/consumed-routes.js'),
  consumedRoutes(pages),
  err => {
    if (err) {
      console.log(err);
    }
    ...
    pages.forEach(page => {
      fs.copyFile(
        ...
        path.join(__dirname, `/components/pages/${page}.js`),
        err => {
          if (err) throw err;
