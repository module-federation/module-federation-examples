...
import { normalizePath, FileData, ProjectGraph, ProjectGraphProjectNode } from '@nrwl/devkit';
...
export async function readProjectExposedModules(
  ...
  try {
    projectFederationConfig =
      (await import(path.join(rootPath, `${projectPath}/federation.config.js`))) || {};
  } catch (e) {
  ...
  for (const [module, modulePath] of moduleExposes) {
    ...
    const moduleRequires: string[] = file.deps
...
function parseNativeFederationModules(
  ...
  const moduleFiles: FileData[] = project.data.files.filter(file => {
  ...
  for (const moduleFile of moduleFiles) {
    ...
    const hasNativeFederationImport = moduleImports.some(nativeImport => {
    ...
    const nativeFederationInvokes: CallExpression[] = moduleExpressions.filter(node => {
    ...
    try {
      ...
      nativeFederationRemotes = [
        ...nativeFederationRemotes,
        ...nativeFederationInvokes
          .map(node => {
...
export function readProjectConsumedModules(
  ...
  const nativeFederationRemotes: (LoadRemoteModuleOptions & { file: string })[] =
    parseNativeFederationModules(rootPath, project);
  ...
  for (const { remoteName, exposedModule, file } of nativeFederationUniqueRemotes) {
    const module: NFPDashboardConsumeModule = modules.find(module => {
