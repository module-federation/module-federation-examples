...
import { createPackageJson, readJsonFile, writeJsonFile, ProjectGraph } from '@nrwl/devkit';
...
import {
  readProjectDependenciesBy,
  readProjectDevDependencies,
...
export async function buildDashboardFile(
  ...
  const rootPackageJson = (
    existsSync('package.json') ? readJsonFile('package.json') || {} : {}
  ) as PackageJson;
  ...
  const dashboard: NFPDashboardOutputFile = {
    ...
    devDependencies: readProjectDevDependencies(
      graph,
      rootPath,
      name,
      projectPackageJson,
    ...
    optionalDependencies: readProjectDependenciesBy(
 ...
 *
 */
export async function sendDashboardFile(
  endpoint: string,
